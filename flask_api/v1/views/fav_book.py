...
@grand_view.route('/<string:user_id>/favs/books', methods=['GET'],
                  strict_slashes=False)
def get_user_fav_books(user_id):
    ''' get all favorite instances from the database by a certain user '''

    favs = [i.to_dict() for i in storage.all(FavoriteBook).values()
...
def create_book_fav():
    ...
    new_fav = FavoriteBook(user_id=request.get_json()["user_id"],
...
@grand_view.route('/favs/books/<string:fav_id>', methods=['DELETE'],
                  strict_slashes=False)
