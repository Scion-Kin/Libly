...
def get_authors():
    ...
    authors = {}
    for i in all:
        book_authors = [j for j in storage.all(BookAuthor).values() if
                        j.author_id == i["id"]]
        name = (i["first_name"] + ' ' + i["middle_name"] + ' '
                + i["last_name"]) if i["middle_name"] is not None else\
...
@grand_view.route('/authors/<string:author_id>', methods=['GET'],
...
def create_author():
    ...
    authors = [i for i in storage.all(Author).values() if i.first_name.lower()
               == request.get_json()["first_name"].lower() and
...
@grand_view.route('/authors/<string:author_id>', methods=['PUT'],
                  strict_slashes=False)
...
@grand_view.route('/authors/<string:author_id>', methods=['DELETE'],
                  strict_slashes=False)
def delete_author(author_id):
    ...
    author = storage.get(Author, author_id)
    if author is not None:
        all_book_rels = [i for i in storage.all(BookAuthor).values() if
