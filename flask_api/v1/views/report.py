...
def get_report(days):
    ...
    ''' here we add thirty days, in the case that
    the current date number is less than days requested. '''

    period = int(today[2]) - days
    period = period + 30 if period < 0 else period

    # get all items from this year
    all = [i for i in storage.all().values()
           if str(i.created_at).split(' ')[0].split('-')[0] == today[0]]

    if days > int(today[2]):
        # get all items from the requested range of days from last month
        if today[1] == '01':
            prev_month = [i.to_dict() for i in all if
                          str(i.created_at).split(' ')[0].split('-')[1] == '12'
                          and str(i.created_at).split(' ')[0].split('-')[2]
                          >= period]
        else:
            prev_month = [i.to_dict() for i in all if
                          int(str(i.created_at).split(' ')[0].split('-')[1])
                          == (int(today[1]) - 1) and
                          int(str(i.created_at).split(' ')[0].split('-')[2])
                          >= period]

        # get all items from this month
        this_month = [i.to_dict() for i in all
                      if str(i.created_at).split(' ')[0].split('-')[1]
                      == today[1]]
        ...
        # get all items from this month
        all = [i for i in all if
               str(i.created_at).split(' ')[0].split('-')[1] == today[1]]

        # get all items from the requested range of days
        all = [i.to_dict() for i in all if period <
               int(str(i.created_at).split(' ')[0].split('-')[2])]
...
def year_report(year):
    ...
    all = [i for i in storage.all().values()
           if int(str(i.created_at).split(' ')[0].split('-')[0]) == year]

    all_months = []

    # get data from all months
    for i in range(1, 13):
        month = [j.to_dict() for j in all
                 if int(str(j.created_at).split(' ')[0].split('-')[1]) == i]
